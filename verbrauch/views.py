...
def edit_action(request, action_id):
    ...
    if request.method == 'POST':
        if 'edit_action' in request.POST:
            ...
            new_level = get_level(request.user)
            if old_level['level_number'] < new_level['level_number']:
                createBenachrichtigung(request, f'Du hast eine Aktion vom Typen {action_type} bearbeitet und bist so ins Level {new_level["current_level"].description} aufgestiegen. &#129395;', request.user)
            elif old_level['level_number'] > new_level['level_number']:
                createBenachrichtigung(request, f'Du hast eine Aktion vom Typen {action_type} bearbeitet, hast dadurch Klimapunkte verloren und bist so ins Level {new_level["current_level"].description} abgestiegen. &#x1F622;', request.user)
            else:
        ...
        if 'delete_action' in request.POST:
            ...
            new_level = get_level(request.user)
            if old_level['level_number'] > new_level['level_number']:
                createBenachrichtigung(request, f'Du hast eine Aktion vom Typen {action_type} gelöscht, hast dadurch Klimapunkte verloren und bist so ins Level {new_level["current_level"].description} abgestiegen. &#x1F622;', request.user)
            else:
